...
class AuxHeadImage(nn.Module):
    def __init__(self, C_in, classes):
        """Auxiliary classifier for image inputs with variable feature sizes."""
        super(AuxHeadImage, self).__init__()
        self.relu1 = nn.ReLU(inplace=True)
...
class FactorizedReduce(nn.Module):
    ...
    def forward(self, x):
        ...
        # path2 = x[:, :, 1:, 1:]
        out = torch.cat([self.path1(x), self.path2(x[:, :, 1:, 1:])], dim=1)
        out = self.bn(out)
        return out
...
Operations = {
    ...
    'sep_conv_3x3': lambda C, stride, affine: SepConv(C, C, 3, stride, 1, shape=None, affine=affine),
    'sep_conv_5x5': lambda C, stride, affine: SepConv(C, C, 5, stride, 2, shape=None, affine=affine),
    'sep_conv_7x7': lambda C, stride, affine: SepConv(C, C, 7, stride, 3, shape=None, affine=affine),
    'dil_conv_3x3': lambda C, stride, affine: DilConv(C, C, 3, stride, 2, dilation=2, shape=None, affine=affine),
    'dil_conv_5x5': lambda C, stride, affine: DilConv(C, C, 5, stride, 4, dilation=2, shape=None, affine=affine),
...
class SepConv(nn.Module):  # 空间可分卷积
    def __init__(self, C_in, C_out, kernel_size, stride, padding, shape, affine=True):
...
Operations_7 = {
    ...
    2: lambda c_in, c_out, stride, shape, affine: DilConv(c_in, c_out, 3, stride, 2, dilation=2, shape=shape,
                                                          affine=affine),
    3: lambda c_in, c_out, stride, shape, affine: DilConv(c_in, c_out, 5, stride, 4, dilation=2, shape=shape,
                                                          affine=affine),
